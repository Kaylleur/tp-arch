...
app.get('/callback', async (req, res) => {
  ...
  if (req.query.code) {
    const response = await fetch(`http://localhost:8081/realms/${realm}/protocol/openid-connect/token`, {
      method: 'POST',
      headers: {
...
app.get('/api/me', async (req, res) => {
  ...
  const response = await fetch('http://localhost:8081/realms/ynov/protocol/openid-connect/token/introspect', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    },
